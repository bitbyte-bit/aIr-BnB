...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS terms_privacy (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    type TEXT UNIQUE, -- 'terms' or 'privacy'
    content TEXT,
...
try { db.exec("ALTER TABLE items ADD COLUMN type TEXT DEFAULT 'product'"); } catch (e) {}
try { db.exec("ALTER TABLE items ADD COLUMN price TEXT"); } catch (e) {}
...
async function startServer() {
  ...
  app.post("/api/auth/signup", async (req, res) => {
    ...
    try {
      ...
      const stmt = db.prepare(
        "INSERT INTO users (email, password, name, role, otp) VALUES (?, ?, ?, ?, ?)"
      );
      const info = stmt.run(email, hashedPassword, name, role, otp);

      // Send OTP to admin
      console.log(`OTP for ${email}: ${otp}`);

      res.status(201).json({ message: "User registered successfully. Share the OTP with the user.", otp });
    } catch (err) {
      console.error(err);
  ...
  app.get("/api/admin/terms", (req, res) => {
    try {
      const terms = db.prepare("SELECT content FROM terms_privacy WHERE type = 'terms'").get();
      res.json({ content: terms?.content || '' });
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  });

  app.put("/api/admin/terms", (req, res) => {
    const { content } = req.body;
    try {
      db.prepare(`
        INSERT OR REPLACE INTO terms_privacy (type, content, updated_at)
  ...
  app.get("/api/admin/privacy", (req, res) => {
    try {
      const privacy = db.prepare("SELECT content FROM terms_privacy WHERE type = 'privacy'").get();
      res.json({ content: privacy?.content || '' });
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  });
